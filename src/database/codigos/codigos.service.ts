...
export class CodigoService {
    ...
    async generateCodeUnique(userId: Types.ObjectId): Promise<number> {
        let codigo: number;
        let exists: Codigo | null;

        do {
            codigo = Math.floor(100000 + Math.random() * 900000);
            exists = await this.model.findOne({ codigo });
        } while (exists);

        const userCode = await this.model.create({ userId, codigo });
