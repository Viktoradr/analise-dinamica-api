...
export const maskAgencia = (str: string) => {
    if (!str || str.trim() === "") return ""
    let strLimpo = Util.cleanCaracteresEspeciais(str)
    let str4digitos = Util.cleanNumber(strLimpo.substring(0, 4) )
    let str1digitos = strLimpo.substring(4, 5).toUpperCase()
    return str4digitos.concat(`-${str1digitos}`)
};
